...
class FileProcessor:
    ...
    @staticmethod
    def check_employee_id(employee_id: str, list_of_objects: list):
        """ Check if the user is trying to input the same employee ID

        :param employee_id: (string) user input:
        :return: (list) of objects
        """
        foundItem_flag = 0  # set controller to 0
        for row in list_of_objects:
            if row[0].lower() == employee_id.lower():
                print("Duplicate employee IDs not allowed")
                foundItem_flag = 1
                break
